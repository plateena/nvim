...
local function build_lines(dir)
  ...
  for _, item in ipairs(files) do
    table.insert(result, string.format("- %s [%s](./%s)", get_icon(item), strip_md(item), item))
  end
  return result
end
...
local function find_markers(lines)
  ...
  for i, line in ipairs(lines) do
    ...
    if trimmed == "<!-- FILE_LIST:START -->" then start_idx = i end
    if trimmed == "<!-- FILE_LIST:END -->" then end_idx = i; break end
  end
...
function M.update_block()
  ...
  local new_block = { "<!-- FILE_LIST:START -->" }
