return {
  ...
  config = function()
    ...
    -- Telescope Copy Path Picker with icons
    ...
    local function copy_path_picker()
      ...
      local entries = {
        { label = "Root Directory", value = cwd, icon = get_icon(cwd) },
        { label = "Full Path", value = full, icon = get_icon(full) },
        { label = "Relative Path", value = relative, icon = get_icon(relative) },
        { label = "Filename", value = filename, icon = get_icon(filename) },
        { label = "Directory", value = directory, icon = get_icon(directory) },
      }

      pickers
        .new({}, {
          prompt_title = "Copy Path",
          finder = finders.new_table({
            results = entries,
            entry_maker = function(entry)
              return {
                value = entry.value,
                display = string.format("%s  %s → %s", entry.icon, entry.label, entry.value),
                ordinal = entry.label .. " " .. entry.value,
                icon = entry.icon,
              }
            end,
          }),
          ...
          attach_mappings = function(prompt_bufnr, map)
            actions.select_default:replace(function()
              ...
              if selection then
                ...
                vim.notify(
                  "✔ Copied: " .. selection.value,
                  vim.log.levels.INFO,
                  { title = "Copy Path", icon = selection.icon }
                )
              end
            end)
            return true
    ...
    vim.keymap.set("n", "<leader>fW", live_grep_under_cursor, { desc = "Live grep word under cursor" })
    vim.keymap.set("n", "<leader>yp", copy_path_picker, { desc = "Copy path picker" })
  end,
}
