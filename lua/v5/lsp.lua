...
local function setup_lsp()
  ...
  local on_attach = function(client, bufnr)
    ...
    if line_count >= MAX_FILE_LINES then
      ...
      vim.notify(
        string.format("LSP '%s' disabled: file has %d lines (limit: %d)", client.name, line_count, MAX_FILE_LINES),
        vim.log.levels.WARN
      )
      return
    end
    ...
    if ft == "ruby" then
      ...
      map("n", "<space>tl", ":!bundle exec rspec %:" .. vim.fn.line(".") .. "<CR>", "Test: Run current line", bufnr)
